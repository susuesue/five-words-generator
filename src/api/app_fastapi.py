...
from src.core.extractor import NeedsExtractor
...
# 辅助函数
def create_response(needs_dict: dict = None, project_id: str = "", project_code: str = "", 
                   project_doc: str = "", inquire_excel: str = "", 
                   error: str = None, code: str = "200") -> StandardResponse:
    if error:
        return StandardResponse(code=code, msg=error, data=None)
    
    data = NeedsData(
        projectId=project_id, projectCode=project_code, projectDoc=project_doc,
        inquire_Excel=inquire_excel, financing=needs_dict.get("融资", ""),
        industry=needs_dict.get("产业", ""), technology=needs_dict.get("技术", ""),
        talent=needs_dict.get("人才", ""), media=needs_dict.get("媒体", "")
    )
...
async def extract_needs(request: ExtractRequest):
    ...
    try:
        if not request.content or not request.content.strip():
            return create_response(error="内容为空", code="400")
        
        needs = extractor.extract(request.content)
        return create_response(needs, request.projectId, request.projectCode, request.projectDoc)
        
    except Exception as e:
        logger.error(f"提取失败: {str(e)}", exc_info=True)
        return create_response(error=str(e), code="500")
...
async def extract_from_backend_files(request: ExtractFilesRequest):
    ...
    try:
        ...
        if not file_path.exists():
            return create_response(error=f"文件不存在: {request.Inquire_Excel}", code="404")
        
        inquiry_content = extract_text(str(file_path))
        combined = f"【问卷调查】\n{inquiry_content}\n\n{'='*70}\n\n【产品文档】\n{request.productDoc}"
        
        needs = extractor.extract(combined)
        return create_response(needs, request.product_Id, request.product_Code, 
                              request.productDoc, inquiry_content)
